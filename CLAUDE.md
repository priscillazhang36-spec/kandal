...
When iterating on dealbreakers, the LLM judge prompt, or the threshold, use the synthetic end-to-end test against the `test_*` tables — Claude in-session judges pairs so no API calls are made. Full reproducible steps: `docs/testing_matching.md`.
